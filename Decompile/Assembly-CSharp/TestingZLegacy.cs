...
public class TestingZLegacy : MonoBehaviour
{
  ...
  private void cycleThroughExamples()
  {
    if (this.exampleIter == 0)
    {
      ...
      {
        this.descrTimeScaleChangeId = LeanTween.value(this.gameObject, 0.01f, 10f, 3f).setOnUpdate((Action<float>) (val => Time.timeScale = val)).setEase(LeanTweenType.easeInQuad).setUseEstimatedTime(true).setRepeat(-1).id;
      }
      else
      {
    ...
    LeanTween.delayedCall(this.gameObject, 1.1f, new Action(this.cycleThroughExamples)).setUseEstimatedTime(this.useEstimatedTime);
  ...
  {
    LeanTween.color(GameObject.Find("LCharacter"), new Color(1f, 0.0f, 0.0f, 0.5f), 0.5f).setEase(LeanTweenType.easeOutBounce).setRepeat(2).setLoopPingPong().setUseEstimatedTime(this.useEstimatedTime);
  }

  public void moveOnACurveExample()
  {
    Debug.Log((object) ("moveOnACurveExample Time:" + Time.time.ToString()));
    LeanTween.move(this.ltLogo, new Vector3[8]
    {
      this.origin,
      this.pt1.position,
      this.pt2.position,
  ...
  {
    Vector3 vector3 = this.ltLogo.transform.position;
    ...
    LeanTween.moveX(this.ltLogo, 0.0f, 0.5f).setEase(this.customAnimationCurve).setDelay(0.5f).setUseEstimatedTime(this.useEstimatedTime);
  }

  public void moveExample()
  {
    Debug.Log((object) nameof (moveExample));
    LeanTween.move(this.ltLogo, new Vector3(-2f, -1f, 0.0f), 0.5f).setUseEstimatedTime(this.useEstimatedTime);
  ...
  public void rotateExample()
  {
    ...
    LeanTween.rotate(this.ltLogo, new Vector3(0.0f, 360f, 0.0f), 1f).setEase(LeanTweenType.easeOutQuad).setOnComplete(new Action<object>(this.rotateFinished)).setOnCompleteParam((object) new Hashtable()
    {
      {
        (object) "yo",
        (object) 5.0
  ...
  public void updateValueExample()
  {
    ...
    LeanTween.value(this.gameObject, new Action<float, object>(this.updateValueExampleCallback), this.ltLogo.transform.eulerAngles.y, 270f, 1f).setEase(LeanTweenType.easeOutElastic).setOnUpdateParam((object) new Hashtable()
    {
      {
        (object) "message",
        (object) "hi"
  ...
  {
    this.ltLogo.transform.eulerAngles = this.ltLogo.transform.eulerAngles with
    {
      y = val
    };
  }
  ...
  public void alphaExample()
  {
    ...
    LeanTween.alpha(gameObject, 0.0f, 0.5f).setUseEstimatedTime(this.useEstimatedTime);
  ...
  public void moveLocalExample()
  {
    ...
    LeanTween.moveLocal(gameObject, new Vector3(0.0f, 2f, 0.0f), 0.5f).setUseEstimatedTime(this.useEstimatedTime);
  ...
  public void loopPause() => LeanTween.pause(GameObject.Find("Cube1"));

  public void loopResume() => LeanTween.resume(GameObject.Find("Cube1"));
