...
public class WarpTextExample : MonoBehaviour
{
  ...
  private void Start() => this.StartCoroutine((IEnumerator) this.WarpText());
  ...
  private IEnumerator WarpText()
  {
    ...
    while (true)
    {
      while (this.m_TextComponent.havePropertiesChanged || (double) old_CurveScale != (double) this.CurveScale || (double) old_curve.keys[1].value != (double) this.VertexCurve.keys[1].value)
      {
        ...
        if (characterCount != 0)
        {
          ...
          for (int index = 0; index < characterCount; ++index)
          {
            if (textInfo.characterInfo[index].isVisible)
            {
              ...
              Vector3 vector3 = (Vector3) new Vector2((float) (((double) vertices[vertexIndex].x + (double) vertices[vertexIndex + 2].x) / 2.0), textInfo.characterInfo[index].baseLine);
